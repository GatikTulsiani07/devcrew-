...
This specification defines the Devcrew platform across the `devcrew`, `devcrew-ui`, `devcrew-backend`, `devcrew-review`, and `devcrew-docs` repositories. It is maintained only in `devcrew-docs`. Repository operating manuals may link to it but must not copy or redefine it.

The code repositories are currently minimal Next.js application scaffolds. The capabilities below define the intended product and its required behavior; they do not imply that those capabilities are already implemented.
...
- Support incremental delivery across independently owned repositories without duplicating product documentation.
...
- Authenticate requests and authorize project, agent, work, document, review, memory, and secret operations at the server boundary.
...
| Repository | Platform responsibility |
| --- | --- |
| `devcrew` | Main integration point, cross-layer reconciliation, final product assembly, release validation, and release readiness. |
| `devcrew-ui` | Pages, components, layout, navigation, themes, responsive behavior, accessibility, and client interactions. It consumes backend contracts and does not own server business rules. |
| `devcrew-backend` | Server routes, services, orchestration, data models, persistence boundaries, validation, authorization, integrations, and operational behavior. It does not own presentation. |
| `devcrew-review` | Independent code, architecture, UX, accessibility, security, performance, and release review. It produces findings and merge-readiness decisions rather than product features. |
| `devcrew-docs` | Sole source of truth for platform specification, architecture, design language, execution plan, and backlog. |
| `Guildly-Reference` | Read-only research input for product and design principles. It is neither a Devcrew implementation repository nor an authoritative feature specification. |
...
- The five Devcrew repositories operate within their documented boundaries and consume this canonical documentation without local copies.
