...
export function ProjectSelector() {
  return (
    <details className="group relative">
      <summary className="mx-3 mt-4 flex min-h-[5.75rem] cursor-pointer list-none items-start gap-3 rounded-[var(--radius-small)] px-3.5 py-3.5 transition-colors hover:bg-panel/55 [&::-webkit-details-marker]:hidden">
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-[0.95rem] font-medium text-ink">{project.name}</span>
          <span className="mt-1.5 block truncate text-[0.76rem] text-ink-muted">{project.repository}</span>
          <span className="mt-1 block text-[0.72rem] text-ink-muted">Local fixture environment</span>
        </span>
